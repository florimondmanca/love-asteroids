...
#### lume.loop(x, min, max)
Returns the number `x` looped between the numbers `min` and `max`.
```lua
lume.loop(1.2, 0, 1) -- Returns 0.2
lume.loop(-0.5, 0, 2) -- Returns 1.5
```
...
#### lume.multilerp(t, y, amount)
Returns the linearly interpolated value of a function `t -> y(t)` represented by its discrete outputs `y` and the associated `t` inputs; the tables `t` and `y` are assumed to be sorted; `amount` should be in the range of `min(t)` - `max(t)`; if `amount` is outside of this range it is clamped.
```lua
lume.multilerp({0, 0.5, 1}, {0, 2, 3}, 0.25) -- Returns 1
lume.multilerp({0, 0.25, 0.5, 1}, {10, -4, 3, 1}, 0.75) -- Returns 2
```
